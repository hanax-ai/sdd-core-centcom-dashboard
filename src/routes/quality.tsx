...
import { workPackageAcceptancePct } from "@/data/metrics";

export const Route = createFileRoute("/quality")({
  head: () => ({
    meta: [
      { title: "Quality & Verification · SDD-Core SITREP — Situation Report" },
      {
        name: "description",
        content: "verify-layout, CI evidence, path/content invariants. 'Unknown' is not 'Passing'.",
      },
      {
        property: "og:title",
        content: "Quality & Verification · SDD-Core SITREP — Situation Report",
      },
      {
        property: "og:description",
        content: "Evidence-graded verification health with explicit unknown states.",
      },
    ],
  }),
...
function QualityPage() {
  ...
  const icon = (s: string) =>
    s === "passed" ? (
      ...
      <XCircle className="h-5 w-5 text-status-blocked" />
    ) : (
      ...
      <Card className="border-status-gate/30 bg-status-gate/5">
        <CardContent className="p-4 text-sm">
          <div className="font-semibold">
            CI policy: {snap.ciPolicy} feedback only, not a required merge gate.
          </div>
          <div className="text-muted-foreground mt-1">
            Current commit-status at HEAD:{" "}
            <span className="font-mono">no current status evidence observed</span>. The dashboard
            displays this literally — it does not claim "passing".
          </div>
        </CardContent>
      </Card>
              ...
              <div className="flex items-center gap-3 text-xs">
                ...
                {r.passedChecks !== undefined && r.totalChecks !== undefined && (
                  <span className="font-mono">
                    {r.passedChecks}/{r.totalChecks} checks
                  </span>
                )}
              </div>
              {r.provenance.parsingWarnings?.map((w) => (
                <div key={w} className="text-[11px] text-status-gate">
                  ⚠ {w}
                </div>
              ))}
            </CardContent>
                ...
                <div key={w.id} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="mt-1 h-1.5 rounded-full bg-muted overflow-hidden">
                      <div
                        className={`h-full ${pct === 100 ? "bg-status-completed" : pct > 0 ? "bg-status-progress" : "bg-status-deferred"}`}
                        style={{ width: `${pct}%` }}
                      />
                    </div>
                  </div>
