...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "Overview · SDD-Core Command Center" },
      { name: "description", content: "Current position, KPIs, next best actions and evidence-backed status for the SDD-Core implementation." },
      { property: "og:title", content: "SDD-Core Command Center — Overview" },
      { property: "og:description", content: "Answer-first executive view of plan progress, gates, defects, and verification." },
    ],
  }),
...
function OverviewPage() {
  ...
  const buckets = {
    completed: pkgs.filter((w) => w.status === "completed").length,
    progress: pkgs.filter((w) => w.status === "in-progress").length,
    awaiting: pkgs.filter((w) => w.status.startsWith("awaiting")).length,
    blocked: pkgs.filter((w) => w.status === "blocked").length,
    deferred: pkgs.filter((w) => w.status === "deferred").length,
    notStarted: pkgs.filter((w) => w.status === "not-started").length,
  };
  const total = pkgs.length;

  const attention = [
    { id: "DEC-01", title: "Approve Gate 2 implementation for skill lifecycle pilot", severity: "high" as const, age: "6 days awaiting Agent Zero", link: "/governance" },
    { id: "DEF-DR1", title: "Deliverables register version drift vs. head commit", severity: "high" as const, age: "Discovered at snapshot", link: "/issues" },
    { id: "WP-O1", title: "Deliverables register reconciliation is blocked", severity: "medium" as const, age: "Blocked on DEC-03", link: "/work-packages" },
    { id: "VR-3", title: "No current CI status evidence observed at HEAD", severity: "medium" as const, age: "Not 'Passing' — Unknown", link: "/quality" },
    { id: "WP-M3", title: "Maturity item 3: lifecycle authoring awaits authorization", severity: "low" as const, age: "21 days awaiting decision", link: "/governance" },
  ];
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent className="relative grid gap-6 lg:grid-cols-4">
          <div className="lg:col-span-2 space-y-1.5">
            ...
            <div className="text-sm text-muted-foreground">
              Framework maturity backlog 7 of 8 completed; skill lifecycle Gate 1 promoted;
              Gate 2 implementation approval pending.
            </div>
          </div>
          <div className="space-y-1.5">
            <div className="text-xs uppercase tracking-widest text-muted-foreground">Verified milestone</div>
            <div className="text-lg font-semibold">WP-R1 shipped</div>
            <div className="text-sm text-muted-foreground font-mono">
              {snap.headSha.slice(0, 7)} · {formatDistanceToNow(new Date(snap.lastSyncedAt), { addSuffix: true })}
            </div>
          </div>
          <div className="space-y-1.5">
            <div className="text-xs uppercase tracking-widest text-status-gate">Primary blocker</div>
          ...
          <div className="lg:col-span-4 flex flex-wrap items-center gap-2 pt-2 border-t">
            <span className="text-xs text-muted-foreground uppercase tracking-widest">Recommended next</span>
            <span className="text-sm font-medium">Route DEC-01 to Agent Zero with lifecycle proposal evidence pack.</span>
            <Button asChild size="sm" variant="ghost" className="ml-auto gap-1">
              <Link to="/governance">Open governance <ArrowRight className="h-3.5 w-3.5" /></Link>
            </Button>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <KpiCard metric={debt} accent="gate" hint={`${gates.filter((g) => g.status === "pending").length} pending gates`} />
        <KpiCard metric={dl} accent="blocked" hint="GitHub issues ≠ all defects" />
        <KpiCard metric={vh} accent="gate" hint="Unknown ≠ Pass" />
        <KpiCard metric={{ ...{ label: "Blocked items", value: buckets.blocked, formula: "work packages in 'blocked' state", scope: `${total} work packages`, observedAt: snap.lastSyncedAt } }} accent="blocked" />
        ...
        <CardContent className="space-y-3">
          <div className="flex h-3 w-full overflow-hidden rounded-full border">
            {[
              ...
              <div key={b.key} className={`${b.cls} h-full`} style={{ width: `${(b.n / total) * 100}%` }} title={`${b.key}: ${b.n}`} />
            ))}
          </div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="relative grid grid-cols-2 gap-3 md:grid-cols-4">
            {phases.map((ph, i) => {
              const tone =
                ph.status === "completed" ? "border-status-completed/40 bg-status-completed/8"
                : ph.status === "current" ? "border-primary/50 bg-primary/8 elev-2"
                : ph.status === "next" ? "border-status-gate/40 bg-status-gate/6"
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <span className="text-[11px] font-mono text-muted-foreground">ranked by severity · authority · age</span>
          </CardHeader>
          <CardContent className="space-y-2">
            {attention.map((a) => (
              ...
              >
                <span className={`h-2 w-2 shrink-0 rounded-full ${a.severity === "high" ? "bg-status-blocked" : a.severity === "medium" ? "bg-status-gate" : "bg-status-deferred"}`} />
      ...
      <Card>
        <CardHeader className="pb-2">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[11px] font-mono text-muted-foreground">commits ≠ business value</span>
          </div>
        </CardHeader>
