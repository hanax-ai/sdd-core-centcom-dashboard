...
2. Under **Require status checks to pass**, **remove** any wrongly-bound entry (e.g. a prefixed `Dashboard CI / required` or bare `Dashboard CI` label sitting as a phantom "Expected — waiting for status to be reported") and add the job-name context **`required`** by **selecting it from the search dropdown of already-observed checks** (do not free-type the string, and do not select the workflow-prefixed label). Then set the check's **source to GitHub Actions** (not "Any source") so only a status reported by the **GitHub Actions integration** can satisfy the gate — this prevents a non-Actions integration (e.g. a bot or third-party app) from reporting a same-named `required` status to bypass enforcement. Note that "GitHub Actions" scopes to the **integration, not to a specific workflow**: any GitHub Actions workflow in this repo that reports a check named `required` would also satisfy the rule. This is safe today because the job name `required` is unique to the `Dashboard CI` workflow — if another workflow ever adds a `required` job, keep the job name unique (or rename this job to a distinctive context) to preserve effective workflow-level scoping.
